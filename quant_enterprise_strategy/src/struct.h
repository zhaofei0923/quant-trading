...
struct KAMA
{
	...
	KAMA()//初始化函数
	{
        ...
        sInstrumentID = "";
		dKama = 0.0;
		dEfficiencyRatio = 0.0;
		dHigh = 0.0;
		dLow = 0.0;
	}

	//赋值函数
	void set(KAMA *kama1, KAMA *kama2)
	{
		kama1->sDate = kama2->sDate;
		kama1->sTime = kama2->sTime;
		kama1->sInstrumentID = kama2->sInstrumentID;
		kama1->dKama = kama2->dKama;
		kama1->dEfficiencyRatio = kama2->dEfficiencyRatio;
		kama1->dHigh = kama2->dHigh;
		kama1->dLow = kama2->dLow;
	}

	//重载赋值运算符
	KAMA &operator=(const KAMA &kama)
	{
		if (this == &kama)
		{
			return *this;
